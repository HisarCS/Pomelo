    ...
    def takeInput(self):
        while True:
            ...
            if (button == True):
                ...
                self.findAruco(self.camera)

            while (
                    self.xx != self.controller.butonlariOku() or self.xy != self.controller.sagVerileriOku() or self.xz != self.controller.solVerileriOku()):
                lx, ly = self.controller.solVerileriOku()
                print(lx, ly)
                rightSpeed, leftSpeed = self.Motors.translateControllerDataToMotorData(lx, ly)
                # leftSpeed = motors.kumandaVerisiniMotorVerilerineCevirme (lx, ly, False)
                print(lx, " ", ly, " ", rightSpeed, " ", leftSpeed)
                self.Motors.adjustSpeed(rightSpeed, leftSpeed)
                sleep(0.3)
                self.Motors.adjustSpeed(0, 0)
            sleep(0.2)
